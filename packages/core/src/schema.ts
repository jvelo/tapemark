...
/**
 * True if `col` is a SQLite-auto-generated primary key — i.e. an INTEGER
 * column that is the sole primary key. SQLite aliases such columns to
 * rowid and fills them in on INSERT when omitted, so admin UIs typically
 * hide the field on the create form.
 */
export function isAutoGeneratedPk(col: Column, primaryKey: string[]): boolean {
  return (
    primaryKey.length === 1 &&
