  ...
  return (
    <form method="post" action={action} class="tm-form" id={formId}>
      ...
      {columns.map((col) => {
        ...
        // SQLite auto-generates INTEGER PKs from rowid. On create, hide the
        // field so the user doesn't accidentally clobber the sequence.
        // Admins can opt back in via the per-column `showOnCreate` flag.
        if (!isEdit && isAutoGeneratedPk(col, primaryKey) && !cc?.showOnCreate) {
          return null;
        }

        // A column is "required" only if NOT NULL and no DEFAULT is set —
        // when a DEFAULT exists, omitting the field lets SQLite fill it in.
        const isRequired =
